...
#Copy used for 20201207 update by RN
...
site_N <- read.csv("Fish/output/fishSAMPLES.20201204.csv", header=TRUE) # new data that is to be added to existing
  #get SITES columns
  site_N<- site_N %>% 
    left_join(site, site_N, by = "siteID") %>% 
    rename(lakeID = "lakeID.y") %>% 
    rename(updateID = "updateID.y") %>% #keeps SITES$updateID, not the data's updateID
    mutate(lakeID = word(site_N$siteID,1,1,sep="_")) %>% 
    mutate(siteName = word(site_N$siteID,2,2,sep="_")) %>% 
    select(colnames(site))
  site_N<- site_N %>% 
    mutate(updateID = case_when(is.na(site_N$updateID)~"siteFishInfo.20201124", TRUE ~ site_N$updateID))


  #fix some site names (error made in my last fish update siteFishInfo2019.20200529)
  site<- site %>%
    mutate(siteName = gsub("wholeShoreline","WholeShoreline",site$siteName)) %>%
    mutate(siteID = gsub("wholeShoreline","WholeShoreline",site$siteID)) %>%
    mutate(updateID = gsub("siteFishInfo2019.20200529", "fish.fix.20201211", site$updateID)) %>% 
    distinct() #site had a duplicate row for SR_FN7, couldn't find why, removed
  site[duplicated(site$siteID),] #these existed before siteFishInfo2019.20200529 so I'll keep the old ones and remove the new ones:
  
  site<- site %>% 
    filter(!(updateID == "fish.fix.20201211" & lakeID %in% c("AQ","FD","HT","JS","LT","TO")))
...
write.table(site_current,file.path(outDir,paste('SITES','.txt',sep='')),quote = F,sep = '|',row.names=F,
            col.names=F) # should be the same format that Access creates
write.csv(site_current,file.path(outDir,'SITES.20201214.csv'), row.names = F) #for rawCSV file storage
#Update Metadata Table
write.table(update_N,file.path(outDir,paste('UPDATE_METADATA','.txt',sep='')),quote = F,sep = '|',row.names=F,
col.names=F)

###---###---### SAMPLE TABLE UPDATE ###---###---###
sample<- dbTable("FISH_SAMPLES") #reads in most recent text file of SAMPLES table
...
sample<-sample %>% 
  #fix sampleID errors:
  mutate(updateID = case_when(grepl("line2",sample$sampleID)~ "fish.fix.20201211", TRUE ~ sample$updateID)) %>% 
  mutate(sampleID = gsub("WholeShoreline2","WholeShoreline_2",sample$sampleID)) 

sample<- sample %>% 
  #fix "21070818" date:
  mutate(updateID = case_when(grepl("21070818", sample$sampleID)~ "fish.fix.20201211", TRUE ~ sample$updateID)) %>% 
...
sample<- sample %>% 
  mutate(updateID = case_when(sample$metadataID == "hyperstability.20180604" ~ "fish.fix.20201211", TRUE ~ sample$updateID)) %>% #this was the stray case_when() that caused the NA's that needed to be fixed in 4.2.1 
...
sample<- sample %>%   
  ...
  mutate(updateID = case_when(sample$siteID %in% c("SR _FNm1", "SR _FN7") ~ "fish.fix.20201211", TRUE ~ sample$updateID)) %>% 
...
sample<- sample %>% 
  ...
  mutate(updateID = case_when(sample$metadataID %in% c("blgMorphologyDOC.20180627", "blgMorphologyDOC.20180628",
                              ...
                              ~ "fish.fix.20201211", TRUE ~ sample$updateID)) %>% 
...
write.csv(sample, file.path(outDir,"samples.fish.fix.20201211.csv"),row.names = F)
...
write.table(sample_current,file.path(outDir,paste('FISH_SAMPLES','.txt',sep='')),quote = F,sep = '|',row.names=F,
            col.names=F) # should be the same format that Access creates 
write.csv(sample_current,file.path(outDir,'FISH_SAMPLES.20201214.csv'), row.names = F) #for rawCSV file storage
...
tab<- dbTable('FISH_INFO') #Choose table you're looking to update #73600
#remove errors in FISH_INFO before proceeding
tab<- tab %>% 
  mutate(updateID = case_when(grepl("21070818", tab$sampleID)~ "fish.fix.20201211", TRUE ~ tab$updateID)) %>% 
  mutate(sampleID = gsub("21070818", "20170818",tab$sampleID)) %>%
  mutate(fishID = gsub("21070818", "20170818", tab$fishID)) %>% 
  add_column(year = substr(word(tab$sampleID,3,3,sep="_"),1,4)) %>% 
  filter(year != "2019") %>% 
  select(colnames(dbTable("FISH_INFO")))
  #73600 rows in dbTable("FISH_INFO") and 70231 in filtered table. Diff = 3369, which is the number added in the erroneous update tableFishInfo2019.20200529
tab<- tab %>% 
  mutate(updateID = case_when(grepl("hyperstability.20180604", tab$sampleID)~ "fish.fix.20201211", TRUE ~ tab$updateID)) %>% 
  mutate(sampleID = gsub("hyperstability.20180604","hyperstability.20180521", tab$sampleID)) %>% 
  mutate(fishID = gsub("hyperstability.20180604","hyperstability.20180521", tab$fishID)) %>% 
  mutate(metadataID = gsub("hyperstability.20180604", "hyperstability.20180521", tab$metadataID))


tab_N <- read.csv ('Fish/output/fishINFO.20201204.csv', header=TRUE, stringsAsFactors = F) # Pull in new table (make sure that csv is saved in working directory folder)
...
checkDuplicates(tab_current, "fishID")
...
#!
outDir<-'Updates/update1' #Will be unique for each user
write.table(tab_current,file.path(outDir,paste('FISH_INFO','.txt',sep='')),quote = F,sep = '|',row.names=F,
            col.names=F)
write.csv(tab_current,file.path(outDir,'FISH_INFO.20201214.csv'), row.names = F) #for rawCSV file storage


#Other updates besides fish in update1:
#removing DOC table and putting it in WATER_CHEM
#Kaija did this and sent me the new WATER_CHEM table (Misc fixes/)
  #From Kaija: To clarify about what this spreadsheet is: it should be treated as the entire DOC portion of the WATER_CHEM table. 
  #(1)So, remove all of the currently-existing DOC measurements from WATER_CHEM
    #--rounding and QC codes may have changed for them. 
  #(2)Then bind_rows([remaining, non-DOC rows of WATER_CHEM], [the DOC data attached here]). 
  #(3)Then WATER_CHEM should be ready for the database update.

#(1) Remove DOC from WATER_CHEM:
wc<- read.csv("Misc fixes/output/WATER_CHEM.20201208.csv", header = T, stringsAsFactors = F) #contains previous updates
wc<- wc %>% 
  filter(!parameter == "DOC") #removes 2852 rows leaving 16892 rows

#(2) Add new QC'd DOC from Kaija's file:
newDOC<- read.csv("DOC/output/QCedDOCData_withMultiples.csv", header = T, stringsAsFactors = F) #3075 rows
newDOC$updateID<-"DOCQC.20201204"
newDOC<- newDOC %>% 
  mutate(updateID = case_when(newDOC$lakeID == "CW" ~ "lakeID.fix.20201208", TRUE ~ newDOC$updateID)) %>% 
  mutate(lakeID = gsub("CW","CY", newDOC$lakeID)) %>% 
  mutate(sampleID = gsub("CW","CY", newDOC$sampleID))

checkCols(wc, newDOC)
newwc<- rbind(wc, newDOC)
newwc<- newwc %>%  arrange(newwc$dateSample, newwc$sampleID)

unique(newDOC$updateID)
unique(newwc$updateID)

#(3) write tables
outDir<-'Updates/update1' #Will be unique for each user
write.table(newwc,file.path(outDir,paste('WATER_CHEM','.txt',sep='')),quote = F,sep = '|',row.names=F,
            col.names=F) 
write.csv(newwc,file.path(outDir,'WATER_CHEM.20201214.csv'), row.names = F) #for rawCSV file storage


#reconciling CHLOROPHYLL updates
#Kaija made changes to CHLOROPHYLL (projectID drag down) AND I made changes (CW to CY lakeID)
#Kaija: chlProjectIDDragdownFix.20201204, Me: lakeID.fix.20201208
#CHLOROPHYLL.txt in Misc Fixes/output doesn't have the 2 "chlProjectIDDragdownFix.20201204" with projectID fixes

kchl<-read.csv("Misc fixes/input/CHLOROPHYLL.20201204.csv",header = T, stringsAsFactors = F)
#occurs @ PE_DeepHole_20160714_1000_Hypo_8_Limno.Sample.20160505
#just going to take mine and make Kaija's changes: 

rchl<-read.csv("Misc fixes/output/CHLOROPHYLL.20201208.csv", header = T, stringsAsFactors = F)
rchl$projectID<-as.character(rchl$projectID)
rchl<- rchl %>% 
  mutate(updateID = case_when(rchl$sampleID =="PE_DeepHole_20160714_1000_Hypo_8_Limno.Sample.20160505" 
                              ~ "chlProjectIDDragdownFix.20201204", TRUE ~ rchl$updateID)) %>% 
  mutate(projectID = case_when(rchl$sampleID =="PE_DeepHole_20160714_1000_Hypo_8_Limno.Sample.20160505"
                               ~ "17", TRUE ~ rchl$projectID))
rchl$projectID<-as.integer(rchl$projectID)
write.table(rchl, file.path(outDir,paste('CHLOROPHYLL','.txt',sep='')),quote = F,sep='|',row.names=F,col.names=F)
write.csv(rchl, file.path(outDir,'CHLOROPHYLL.20201214.csv'), row.names = F)  

#lakeID misc. table updates:
#CHLOROPHYLL: updated above
#COLOR
  dbColor<-dbTable("COLOR")
  color<-read.csv("Misc fixes/output/COLOR.20201208.csv", header = T, stringsAsFactors = F)  
  write.table(color, file.path(outDir,paste('COLOR','.txt',sep='')),quote = F,sep='|',row.names=F,col.names=F)
#SAMPLES
  sampFix<-read.csv("Misc fixes/output/SAMPLES.20201208.csv", header = T, stringsAsFactors = F)  
  write.table(sampFix, file.path(outDir,paste('SAMPLES','.txt',sep='')),quote = F,sep='|',row.names=F,col.names=F)
#SITES: updated above
#WATER_CHEM: updated above
...
#update versions_history once all other updates are complete but before running sql script to make new database file that will be shared.
vers<-dbTable("version_history")
write.csv(vers, file="Updates/update1/versionHistory_current_fix.csv", row.names = FALSE) # open csv, make updates/changes, bring back to R to write txt file
vers_current <- read.csv("Updates/update1/versionHistory_current_fix.csv", header = T, stringsAsFactors = F)
write.table(vers_current,file.path(outDir,'VERSION_HISTORY.txt'),quote = F,sep = '|',row.names=F, col.names=F)
