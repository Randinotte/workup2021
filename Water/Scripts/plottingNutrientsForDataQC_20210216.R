...
newdata<-read.csv("Water/Output/compiledData/nutrients_20210217.csv",header = TRUE, stringsAsFactors = F) #comes from the CompileXYZ.R script
...
#nutrients_check.csv
#write.csv(check,"Water/Output/checkCSVs/nutrients_check.csv")




plot(plot_list[[104]]) #how to plot just one plot from the list
###### End of big for loop
...
for(p in 1:nrow(site)){
  ...
  cowplot::save_plot(file.path("Water/QC plots/Nutrients",paste(site$lakeID[p], site$depthClass[p],site$siteName[p],"Nutrient_plots.pdf",sep="_")), plots, base_height = 5, base_width = 10)
